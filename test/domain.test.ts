...
import { baselineLineDigest, buildDiffRecords, digestBytes, fileStatus, physicalLines, reviewCounts, terminalPayload, type FileRecord, type RawGitHunk } from '../src/domain.ts';
const bytes = (value: string): Uint8Array => new TextEncoder().encode(value);
function record(baseline: string, current: string, hunks: readonly RawGitHunk[]): FileRecord {
...
test('replacement is represented as independent deletion and addition', () => {
    ...
    assert.deepEqual(result.currentLines.map(line => line.changeType), ['added']);
    assert.equal(result.deletedLines.length, 1);
    assert.deepEqual(result.hunks[0], { oldStart: 1, oldCount: 1, newStart: 1, newCount: 1 });
...
test('unchanged ranges are filled around additions and deletions', () => {
    ...
    assert.deepEqual(result.currentLines.map(line => [line.line, line.changeType]), [[1, 'unchanged'], [2, 'added'], [3, 'unchanged']]);
...
test('deleting all content produces deletion records without phantom current lines', () => {
    const result = buildDiffRecords(bytes('one\n\n'), new Uint8Array(), [{ oldStart: 1, oldCount: 2, newStart: 0, newCount: 0 }]);
    assert.equal(result.currentLines.length, 0);
    assert.deepEqual(result.deletedLines.map(line => line.baselineLine), [1, 2]);
});
...
test('addition review state transfers by digest and occurrence when duplicate cardinality is stable', () => {
    ...
    const previous = {
        ...record('', 'same\nsame\n', [{ oldStart: 0, oldCount: 0, newStart: 1, newCount: 2 }]),
        currentLines: initial.currentLines.map((line, index) => index === 0
            ? { ...line, reviewStatus: 'reviewed' as const, lastReviewer: { name: 'Ada', time: 'now' } }
            : { ...line, reviewStatus: 'inReview' as const, lastReviewer: { name: 'Ada', time: 'now' } })
    };
    const next = buildDiffRecords(new Uint8Array(), bytes('same\nsame\n'), [{ oldStart: 0, oldCount: 0, newStart: 1, newCount: 2 }], previous);
...
test('ambiguous duplicate additions reset pending when their cardinality changes', () => {
    ...
    const previous: FileRecord = {
        ...record('', 'same\nsame\n', [{ oldStart: 0, oldCount: 0, newStart: 1, newCount: 2 }]),
        currentLines: initial.currentLines.map((line, index) => index === 0
            ? { ...line, reviewStatus: 'reviewed', lastReviewer: { name: 'Ada', time: 'now' } }
            : line)
    };
    const next = buildDiffRecords(new Uint8Array(), bytes('same\n'), [{ oldStart: 0, oldCount: 0, newStart: 1, newCount: 1 }], previous);
...
test('reviewed additions survive line movement but edited additions reset pending', () => {
    ...
    const reviewed: FileRecord = {
        ...initial,
        currentLines: initial.currentLines.map(line => line.changeType === 'added'
            ? { ...line, reviewStatus: 'reviewed', lastReviewer: { name: 'Ada', time: 'now' } }
            : line)
    };
    const moved = buildDiffRecords(bytes('a\nb\n'), bytes('a\nx\nb\n'), [{ oldStart: 1, oldCount: 0, newStart: 2, newCount: 1 }], reviewed);
    assert.equal(moved.currentLines[1]?.reviewStatus, 'reviewed');
    const edited = buildDiffRecords(bytes('a\nb\n'), bytes('a\ny\nb\n'), [{ oldStart: 1, oldCount: 0, newStart: 2, newCount: 1 }], reviewed);
...
test('removed additions disappear and restored deletions become unchanged', () => {
    ...
    const withoutAddition = buildDiffRecords(bytes('a\n'), bytes('a\n'), [], added);
    assert.equal(withoutAddition.currentLines.some(line => line.changeType === 'added'), false);
    ...
    const stillDeleted = buildDiffRecords(bytes('gone\nkeep\n'), bytes('keep\n'), [{ oldStart: 1, oldCount: 1, newStart: 0, newCount: 0 }], reviewedDeletion);
    assert.equal(stillDeleted.deletedLines[0]?.reviewStatus, 'reviewed');
    const restored = buildDiffRecords(bytes('gone\nkeep\n'), bytes('gone\nkeep\n'), [], reviewedDeletion);
    assert.equal(restored.deletedLines.length, 0);
    assert.ok(restored.currentLines.every(line => line.changeType === 'unchanged' && line.reviewStatus === 'reviewed'));
});
