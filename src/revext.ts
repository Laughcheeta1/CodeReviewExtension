...
const lineComments = new Map<string, string>([
    ...
    ['swift', '//'], ['kotlin', '//'], ['scala', '//'], ['dart', '//'], ['php', '//'], ['fsharp', '//'],
    ['groovy', '//'], ['objective-c', '//'], ['objective-cpp', '//'], ['solidity', '//'],
    ['python', '#'], ['ruby', '#'], ['shellscript', '#'], ['powershell', '#'], ['r', '#'], ['julia', '#'],
...
export function revExtEdits(lines: readonly string[], addedLines: ReadonlySet<number>, languageId: string, nextId: number): {
    ...
    for (const matching of groups.values()) {
        ...
        for (const line of matching) {
            ...
            edits.push({ line, suffix: `${text.length === 0 ? '' : '  '}${token} RevExt: ${nextId}` });
            nextId += 1;
        }
    }
    return { edits, nextId };
