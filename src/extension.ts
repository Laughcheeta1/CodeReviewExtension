...
export async function activate(context: vscode.ExtensionContext): Promise<void> {
    ...
    context.subscriptions.push(
        ...
        vscode.workspace.onDidSaveTextDocument(document =>
            runLogged(log, 'Saved-file reconciliation', service.reconcileSavedDocument(document))
        ),
        service.onDidPromote(source =>
            runLogged(log, 'Closing promoted diff tabs', closePromotedDiffTabs(source))
        ),
        ...
        vscode.commands.registerCommand('codeReviewTracker.markHunkPending', (command: HunkCommand) =>
            markHunk(service, git, command)
        ),
        vscode.commands.registerCommand('codeReviewTracker.markHunkReviewed', (command: HunkCommand) =>
            markHunk(service, git, command)
        ),
        vscode.commands.registerCommand('codeReviewTracker.openReviewDiff', (uri?: vscode.Uri) =>
            openReviewDiff(service, uri)
        ),
        vscode.commands.registerCommand('codeReviewTracker.initializeReviewed', () =>
        ...
        vscode.commands.registerCommand('codeReviewTracker.sendSelectionToTerminal', () =>
    ...
    for (const folder of vscode.workspace.workspaceFolders) {
        ...
        context.subscriptions.push(watcher, deletion);
    }
...
function sendSelection(service: ReviewService): void {
    ...
    if (terminal === undefined) {
        ...
        terminal = folder === undefined
            ? vscode.window.createTerminal({ name: 'Code Review Agent' })
            : vscode.window.createTerminal({ name: 'Code Review Agent', cwd: folder.uri });
        terminal.show(true);
...
async function eligibleWorkspacePaths(folder: vscode.WorkspaceFolder): Promise<readonly string[]> {
    ...
    const uris = await vscode.workspace.findFiles(new vscode.RelativePattern(folder, '**/*'), excluded);
